...
public class JMSFlightStore implements FlightStore {
    ...
    public JMSFlightStore(String brokerUrl, String topicName, String clientId) {
    ...
    @Override
    public void save(Flight flight) throws StoreException {
        try (Connection connection = createConnection()) {
            connection.setClientID(clientId);
            connection.start();
            Session session = createSession(connection);
            Topic topic = createTopic(session);
            String jsonMessage = flightToJson(flight);
            sendMessageToTopic(session, topic, jsonMessage);
            System.out.println("Lo haces?");
    ...
    private void sendMessageToTopic(Session session, Topic topic, String jsonMessage) throws JMSException {
        ...
        System.out.println("Message sent to the topic: " + jsonMessage);
    }
